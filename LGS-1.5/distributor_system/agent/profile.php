...
<head>
  ...
  <title>Profile | Lead System</title>
  ...
  <aside class="sidebar-nav-wrapper active">
    ...
    <nav class="sidebar-nav">
      <ul>
        ...
        <li class="nav-item">
          <a href="profile.php" class="active"><span class="text">Profile</span></a>
        </li>
      </ul>
    </nav>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      form.addEventListener('submit', function (e) {
        ...
        // If password field is empty, just proceed
        if (pwd === '') return;

        // Client-side minimum length check (matches the UI hint)
        if (pwd.length < 3) {
          e.preventDefault();
        ...
        // Extra confirmation popup to prevent accidental password changes
        const confirmMsg = 'You are about to change your password. This will update your account credentials. Are you sure you want to continue?';
        if (!confirm(confirmMsg)) {
          e.preventDefault();
          // keep focus in the password box so user can edit or clear it
          passwordInput.focus();
          return;
        }

        // if confirmed, form will be submitted normally
      });
    });
  </script>
</body>
