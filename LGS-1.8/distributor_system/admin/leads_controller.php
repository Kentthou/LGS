...
function bind_stmt_params(mysqli_stmt $stmt, string $types, array $params): bool {
    // build array of references: first element is types string
    $refs = [];
    $refs[] = $types;
...
// NEW: Remove global duplicates (keep oldest MIN(id) per normalized number+company)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remove_duplicates'])) {
    verify_csrf_token($_POST['csrf'] ?? '');

    $subquery = "(SELECT MIN(id) FROM leads GROUP BY LOWER(TRIM(`number`)), LOWER(TRIM(`company_name`)))";
    $count_sql = "SELECT COUNT(*) AS cnt FROM leads WHERE id NOT IN $subquery";
    $res = $db->query($count_sql);
    $to_delete = $res->fetch_assoc()['cnt'] ?? 0;

    if ($to_delete > 0) {
        $del_sql = "DELETE FROM leads WHERE id NOT IN $subquery";
        $res = $db->query($del_sql);
        $affected = $db->affected_rows;
        $_SESSION['flash_msg'] = "Removed $affected duplicate leads globally, keeping oldest.";
    } else {
        $_SESSION['flash_msg'] = "No duplicates found.";
    }

    header("Location: leads.php?agent_id={$selected_agent}&date={$selected_date}&mode={$mode}&page=1&status_filter={$status_filter}");
    exit;
}
...
if ($selected_agent > 0) {
    ...
    // If dedup requested, we select one id per normalized (number + company) using a subquery grouped by normalized fields.
    if ($mode === 'paged') {
        if ($dedup) {
            // Count distinct (number,company) matching filter
            ...
            bind_stmt_params($stmt, $filter_params[0], array_slice($filter_params, 1));
            $stmt->execute();
            ...
            $sql = "SELECT l.id, l.number, l.company_name, l.description, l.status, l.notes, l.lead_date, l.created_at, l.updated_at
                    FROM leads l
                    INNER JOIN (
                      SELECT MIN(id) AS id
                      FROM leads
            ...
            // non-dedup paged (existing behavior)
